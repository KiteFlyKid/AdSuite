...
parser = argparse.ArgumentParser(description='Ad-Gator')
...
def determine_api_level(dir_base, configs):
    """
    Determines the min and max Android API level of an app and chooses an API version from configs.

    Args:
    dir_base (str): The base directory containing the apktool output.
    configs (list): A list of available API levels in the format ['android-18', 'android-20', ..., 'android-34'].

    Returns:
    tuple: A tuple containing the minSdkVersion, maxSdkVersion, and the chosen API level.
    """
    # Construct the path to the apktool.yml file
    yaml_path = os.path.join(dir_base, "apktool.yml")

    def apk_info_constructor(loader, node):
        # You can choose to return a simple dictionary, or create a custom object for this data.
        return loader.construct_mapping(node, deep=True)

    # Read the apktool.yml file
    with open(yaml_path, 'r') as file:
        yaml.SafeLoader.add_constructor('tag:yaml.org,2002:brut.androlib.apk.ApkInfo', apk_info_constructor)
        apktool_yaml = yaml.safe_load(file)

    # Extract minSdkVersion and targetSdkVersion
    sdkInfo=apktool_yaml.get('sdkInfo')
    min_sdk = sdkInfo.get('minSdkVersion')
    target_sdk = sdkInfo.get('targetSdkVersion')  # Use minSdkVersion if targetSdkVersion is not specified


    available_apis = [18,20,21,23,25,31,32,33,34]

    # Filter APIs that are within the min and max SDK version range
    valid_apis = [api for api in available_apis if int(min_sdk) <= api <= int(target_sdk)]
    if 23 in valid_apis:
        return 23
    else:
    # Choose the closest API to the target SDK, preferring the higher number closer to target_sdk
        return  max(valid_apis, default=None) if valid_apis else 23
...
def invoke_gator_on_apk(
    ...
    # cp_jars = extract_jar_libs(os.path.join(soot_android_path, "build/libs/"))
    cp_jars = extract_jar_libs( "out/artifacts/gator_jar/")
    # cp_jars=cp_jars+extract_jar_libs("commons/build/libs/")
    # cp_jars=cp_jars+extract_jar_libs(os.path.join(soot_android_path, "lib"))
    ...
    cmd = ["java", "-Xmx64G",
           ...
           '-libraryPackageListFile', os.path.join(soot_android_path, 'sootandroid', 'libPackages.txt'),
...
def main():
    ...
    for i in range(start_index, stop_index):
        ...
        configs = parse_gator_params([app_path, "-client", "AdWidgetIDPrinterClient"])
