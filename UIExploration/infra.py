...
class Event:
    ...
    def genAllRawEvents(widget, dull=True) -> List["Event"]:
        widget = Event.ensureWidget(widget)
        # if 'adView' in widget.resourceId:
        #     print('f')
        return [Event(widget, a) for a in widget.actionTypes] if not dull or widget.actionTypes else [Event(widget, "")]
...
class TestCase:
    ...
    def dump(self, path: Path):
        ...
        with open(path / "body.json", "w") as f:
            json.dump([e.dumpAsDict() for e in self._events if e is not None], f)
...
class AdTestCase(TestCase):
    def __init__(self, events=EventSeq([]), ads=None,all_events=None,hierarchies=None):
        if hierarchies is None:
            hierarchies = []
        self._events = EventSeq(events)
        self._all_events=EventSeq(all_events)
        self._hierarchies = []
        self._ad = ads
        self._hierarchy_strings = []
        for hierarchy in hierarchies:
    ...
    def dump(self, path: Path):
        ...
        with open(path / "body.json", "w") as f:
            json.dump([e.dumpAsDict() for e in self._events if e is not None], f)
        # with open(path / "all_events.json", "w") as f:
        #     json.dump([e.dumpAsDict() for e in self._all_events if e is not None], f)
        with open(path / "ad_events.json", "w") as f:
            json.dump([e.dumpAsDict() for e in self._ad], f)
        for idx, hierarchy in enumerate(self._hierarchies):
            hierarchy.dump(path / f"body{idx}.xml")