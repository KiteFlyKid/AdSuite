...
def get_package_name(apk) -> str:
    ...
    if type(apk) is not str:
        ...
        apk_path = Path(configs.apk_dir)/f"{configs.apk_info[apk]['sha']}.apk"
    return subprocess.check_output(["aapt", "dump", "badging", apk_path],shell=True).decode().split("package: name='")[1].split("'")[0]
...
def install_apk(pkg:str):
    sha=configs.apk_info[pkg]['sha']
    print(f"adb install -r {configs.apk_dir}/{sha}.apk")
    os.system(f"adb install -r {configs.apk_dir}/{sha}.apk")
def uninstall_pkg(pkg:str):
    os.system(f"adb uninstall {pkg}")

def check_installed(pkg) -> bool:
...
def ensure_reinstalled(apk:str, pkg:str = None):
    ...
    if check_installed(apk, pkg):
        uninstall_pkg(pkg)
    return ensure_installed(pkg)
def uninstall_app(apk:str, pkg:str = None):
    pkg = pkg if pkg else get_package_name(apk)
    uninstall_pkg(pkg)
def start_app(pkg:str = None):
    # pkg = pkg if pkg else get_package_name(apk)
    print(pkg)
