...
def isVisible(node):
    # if node.attrib.get('resource-id')!=None and 'adView' in node.attrib.get('resource-id'):
    #     print('f')
    return any(node.get(p) == "true" for p in ['focusable', 'visible-to-user'])
...
def havingResourceIdChidenClickable(root):
    # list_group = ["android.widget.RelativeLayout", "android.view.ViewGroup"]
    # for elem in filter(lambda x: x.get('clickable') == "false", root.iter()):
...
def pass_text_from_sibling(root):
        ...
        for i, elem in enumerate(children):
            if elem.get('clickable') == "true":
                ...
                for sibling in children:
                    if sibling != elem and sibling.get('text') and sibling.get('clickable') != "true":
                        sibling_text = sibling.get('text')
                        elem.set("text", original_text + ' ' + sibling_text)
                        break
...
def parseUIHierarchy(root, _filter=isInteractable, _reformClickable=True, app_name=None) -> list:
    ...
    # ret = [Widget(elem) for elem in root.iter() if _filter(elem) or elem.attrib.get('resource-id')!='']
    for widget in ret:
        widget.fixTextEdit()
    return ret
# [Widget(elem) for elem in root.iter() if _filter(elem) or elem.attrib.get('resource-id')!='']
# [Widget(elem) for elem in root.iter() if  elem.attrib.get('resource-id')!='']
...
class SemanticHierarchy(Hierarchy):
    ...
    def __init__(self, pkg_name, app_name, _rawHierarchy = None, backup=None):
        ...
        self._raw_widgets = list(filter(lambda x: x.package in [self.pkg_name, "com.android.packageinstaller"],
                    parseUIHierarchy(_rawHierarchy,_reformClickable=False)))
    ...
    def getRawEvents(self) -> List[Event]:
        if self._raw_events is None:
            self._raw_events=sum([Event.genAllRawEvents(widget) for widget in self._raw_widgets], [])
